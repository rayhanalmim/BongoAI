...
import { Send, Bot, User, Loader2, Copy, Check, Plus, MessageSquare, Image, Video, Upload, X, Menu, Sun, Moon, Sparkles, Zap, Camera } from 'lucide-react';
...
export default function ChatInterface() {
    ...
    const [selectedModel, setSelectedModel] = useState<ModelKey>('claude-opus-4');
    ...
    return (
        <div className="flex h-screen gradient-bg">
            {/* Sidebar */}
            <div className={`${sidebarOpen ? 'w-80' : 'w-0'} transition-all duration-300 overflow-hidden bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-r border-slate-200/50 dark:border-slate-700/50 flex flex-col shadow-elegant`}>
                {/* Sidebar Header */}
                <div className="p-6 border-b border-slate-200/50 dark:border-slate-700/50">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {Object.entries(modelCategories).map(([key, label]) => (
                                <button
                                    ...
                                    className={`p-3 text-xs rounded-xl border transition-all duration-200 flex flex-col items-center justify-center space-y-1 hover:scale-105 ${getCategoryColors(key as CategoryKey, selectedCategory === key)}`}
                                >
                    ...
                    <button
                        ...
                        className="w-full mt-6 p-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white rounded-xl transition-all duration-200 flex items-center justify-center space-x-2 shadow-lg hover:shadow-xl transform hover:scale-105"
                    >
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {filteredConversations.length === 0 ? (
                        <div className="text-center py-8">
                            <div className={`p-3 rounded-xl w-12 h-12 mx-auto mb-3 flex items-center justify-center ${selectedCategory === 'text' ? 'bg-blue-100 dark:bg-blue-900/30' :
                        ...
                        filteredConversations.map((conversation) => (
                            <div
                                ...
                                className={`p-4 rounded-xl cursor-pointer transition-all duration-200 group hover:scale-[1.02] animate-fadeIn ${currentConversationId === conversation.id
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center space-x-2 mb-2">
                                            <div className={`p-1.5 rounded-lg ${conversation.category === 'text' ? 'bg-blue-100 dark:bg-blue-900/30' :
            ...
            <div className="flex-1 flex flex-col">
                {/* Header */}
                <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl shadow-soft border-b border-slate-200/50 dark:border-slate-700/50 p-4">
                            ...
                            <div className="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-slate-600 dark:text-slate-400">
                                        {currentConversation ? `${modelCategories[currentConversation.category]} • ${currentConversation.title}` : 'Multi-modal AI Assistant'}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <select
                                    ...
                                    className="px-3 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent text-slate-900 dark:text-slate-100 transition-all duration-200"
                                >
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <input
                                    ...
                                    className="w-20 px-2 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent text-slate-900 dark:text-slate-100 transition-all duration-200"
                                />
                            </div>
                        </div>
                    </div>
                        ...
                        {messages.length === 0 ? (
                            <div className="text-center py-16 animate-fadeIn">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
                                    <div className="p-6 bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-2xl shadow-soft border border-slate-200/50 dark:border-slate-700/50">
                                        ...
                                        <p className="text-sm text-slate-600 dark:text-slate-400">{bedrockModels[selectedModel].description}</p>
                                    </div>
                                    <div className="p-6 bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-2xl shadow-soft border border-slate-200/50 dark:border-slate-700/50">
                                        <div className="flex items-center space-x-3 mb-3">
                                            <div className={`w-5 h-5 rounded-full ${selectedCategory === 'text' ? 'bg-blue-600' :
                                ...
                                >
                                    <div className={`flex max-w-3xl ${message.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                        <div className={`flex-shrink-0 ${message.role === 'user' ? 'ml-4' : 'mr-4'}`}>
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-soft ${message.role === 'user'
                                        ...
                                        <div className={`relative group ${message.role === 'user'
                                            ...
                                            } rounded-2xl px-6 py-4 shadow-elegant border border-slate-200/50 dark:border-slate-700/50 hover:shadow-lg transition-all duration-200`}>
                                            ...
                                            {message.imageUrl && (
                                                <div className="mb-4">
                                                    <img
                                                        ...
                                                        className="max-w-full h-auto rounded-xl max-h-64 object-cover shadow-soft"
                                                    />
                                                </div>
                                            )}
                            ...
                            <div className="flex justify-start animate-fadeIn">
                                <div className="flex max-w-3xl">
                                    <div className="flex-shrink-0 mr-4">
                                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-slate-600 to-slate-700 dark:from-slate-700 dark:to-slate-800 flex items-center justify-center shadow-soft">
                                            <Bot className="w-5 h-5 text-white" />
                                        </div>
                                    </div>
                                    <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl px-6 py-4 shadow-elegant border border-slate-200/50 dark:border-slate-700/50">
                                        <div className="flex items-center space-x-3">
                ...
                <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-t border-slate-200/50 dark:border-slate-700/50 p-6">
                    <div className="max-w-4xl mx-auto">
                        ...
                        {uploadedImage && (
                            <div className="mb-4 relative inline-block animate-fadeIn">
                                <img
                                    ...
                                    className="max-h-32 rounded-xl border border-slate-200 dark:border-slate-700 shadow-soft"
                                />
                                <button
                        ...
                        <div className="relative">
                            <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-elegant border border-slate-200 dark:border-slate-700 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent transition-all duration-200">
                                <textarea
                                    value={input}
                                    ...
                                    placeholder={`Type your ${selectedCategory === 'text' ? 'message' : `${selectedCategory} prompt`}... (Press Enter to send, Shift+Enter for new line)`}
                                    className="w-full px-6 py-4 pr-24 bg-transparent text-slate-900 dark:text-slate-100 placeholder-slate-500 dark:placeholder-slate-400 resize-none focus:outline-none rounded-2xl"
                                    rows={1}
                                ...
                                <div className="absolute right-2 bottom-2 flex items-center space-x-2">
                                    ...
                                    {(selectedCategory === 'image' || selectedCategory === 'video') && (
                                        <button
                                            ...
                                            className="p-2.5 text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-all duration-200"
                                            title="Upload image"
                                        >
                                    ...
                                    <button
                                        ...
                                        className="p-2.5 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 disabled:from-slate-400 disabled:to-slate-500 text-white rounded-xl disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-105 disabled:hover:scale-100 shadow-lg"
                                    >
